...
Before generating any downstream work (stories, tasks, derived requirements, etc.) from requirements, check their status. If ANY source requirements are NOT Approved, warn the user:
...
- ALL created artifacts go into a descriptive folder (auto-created per session)
...
| `get_link_types` | List link types for a project | project_identifier |
